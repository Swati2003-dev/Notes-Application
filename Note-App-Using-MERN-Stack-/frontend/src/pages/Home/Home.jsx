...
const Home = () => {
  ...
  const getAllNotes = async () => {
    try {
      const res = await axios.get((import.meta.env.VITE_BACKEND_URL || "http://localhost:3001") + "/api/note/all", {
  ...
  const deleteNote = async (data) => {
    ...
    try {
      const res = await axios.delete(
        (import.meta.env.VITE_BACKEND_URL || "http://localhost:3001") + "/api/note/delete/" + noteId,
        { withCredentials: true },
      );
  ...
  const onSearchNote = async (query) => {
    try {
      const res = await axios.get((import.meta.env.VITE_BACKEND_URL || "http://localhost:3001") + "/api/note/search", {
        params: { query },
        withCredentials: true,
      });
  ...
  const updateIsPinned = async (noteData) => {
    ...
    try {
      const res = await axios.put(
        (import.meta.env.VITE_BACKEND_URL || "http://localhost:3001") + "/api/note/update-note-pinned/" + noteId,
